...
int main(int argc, char* argv[]) {
  ...
  if (visualize != ' ') {
	  ...
	  std::vector<std::vector<int> > visual_chars; 

	  visual_chars = std::vector<std::vector<int> >(board.getRows(), std::vector<int>(board.getCols(),(int) '.'));
	  

	  //Visualize function call
	  board.reachability(visual_chars, max_moves, visualize, 1);
	  for (int i = 0; i < visual_chars.size(); ++i) {
		  for (int j = 0; j < visual_chars[i].size(); ++j) {
			  if (visual_chars[i][j] == '.') {
				  std::cout << (char)visual_chars[i][j] << " ";

			  }
			  else {
				  std::cout << visual_chars[i][j] << " ";
			  }
	...
	std::cout << std::endl; 
	  ...
	  board.solve_board(max_moves, 1);
	  board.output_solved_puzzle(board.endings);
  }
