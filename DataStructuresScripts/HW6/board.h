...
public:
  ...
  bool moveRobot(int i, const std::string &direction, std::string is_reach);
  //Self-made robot position modifier
  void setRobotPosition(int i, int row, int col, std::string is_reach); 
  ...
  //Hand-Made function
  void reachability(std::vector<std::vector<int> > &visual_chars, const int &max_moves, const char &visualize, int current_turn);
  void testrun();
  void solve_board(const int &max_moves, int current_turn);
  void output_solved_puzzle(const std::vector<std::vector<std::string> > &endings);
  std::vector<std::string> steps;
  std::vector<std::vector<std::string> > endings;
private:
